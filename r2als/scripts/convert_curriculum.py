...
class CsvToModel:
    ...
    def __init__(self, header_format=None):
        self.COMMENT_SYMBOL = "#"
        self.SPLIT_SYMBOL = ":"
        ...
        if header_format is None:
            ...
            self.HEADER_LISTS = header_format
                    ...
                    for j in range(len(keys)):
                        if row[j] != '':
                            if multiField[j] == True :
                                ...
                                if self.has_header and keys[j] == 'studied_group' and row[j] not in info['studied_groups']:
...
if __name__ == '__main__':
    ...
    lists = CsvToModel(['faculty','department','year','required_num_year','num_semester','max_year','*not_force_enrolled_semesters','*subject_groups','*branches','*categories','*prerequisites','*not_fix_types','*min_credit_each_types']).process(path)
    pp.pprint(lists)
    f_out.close()
